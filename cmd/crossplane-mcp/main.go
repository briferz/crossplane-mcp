...
4. list_providers / list_functions / list_configurations — when a diagnose suspect carries a cryptic provider/function error, or a managed resource never syncs: check whether the package behind it is unhealthy — full condition messages, the failing revision (whose name is by default its runtime Deployment's name), pull-error events, and upgrade skew (failed unpack, awaiting manual approval).
