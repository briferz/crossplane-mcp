...
| `list_providers` | Providers + revisions + health: the escalation when an MR's error is cryptic (crashlooping provider pod, failed unpack, un-approved upgrade). | Rows `{apiVersion, kind, name, package, state, installed, healthy, currentRevision, revisionCount}`; failing ones add full `reasons`, `skew` sentences, `lifecycle`, per-revision health rows, and events. `package` is the verbatim `spec.package` OCI ref — deliberately no parsed `version` field (tag parsing would be wrong for digest pins/ImageConfig rewrites). |
